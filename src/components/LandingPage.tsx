  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-primary/10">
      <header className="border-b bg-background/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="landing-page-shell flex items-center justify-between px-4 py-3.5 sm:px-8 sm:py-4">
      ...
      <section className="landing-page-shell landing-section px-4 sm:px-8">
        <div className="landing-hero-panel mx-auto max-w-4xl text-center">
          ...
          <p className="landing-hero-copy mx-auto mb-9 max-w-3xl text-muted-foreground">
            Review and replay run output in the browser with bundled sample sessions—no server required.
          </p>
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row sm:gap-4">
            <Button size="lg" className="px-8" onClick={onEnterWorkspace}>
              Open workspace
              <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
            <Button size="lg" variant="outline" className="px-8" onClick={onViewDemo}>
              Open Preview
            </Button>
          </div>
        ...
        <div className="landing-feature-grid mt-10">
          <Card className="landing-feature-card">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Bundled JSON aligned to the runner event contract, playable offline.</p>
          </Card>
          <Card className="landing-feature-card">
            <div className="text-sm font-semibold">Local workspace</div>
            <p className="mt-2 text-sm text-muted-foreground">A local profile is stored in your browser—no account server.</p>
          </Card>
          <Card className="landing-feature-card">
            <div className="text-sm font-semibold">Optional API</div>
            <p className="mt-2 text-sm text-muted-foreground">If you connect a CrewForge API and use a server-issued token, live streams can be enabled.</p>
          </Card>
        </div>

        <Card className="landing-note-panel mt-10 p-6 sm:p-8">
      ...
      <footer className="border-t bg-card py-8">
        <div className="landing-page-shell px-4 sm:px-8">
          <div className="text-center text-sm text-muted-foreground">
            ...
            <p className="mt-1.5">Replay mode is the default; no backend is required.</p>
          </div>
        </div>
      </footer>
    </div>
